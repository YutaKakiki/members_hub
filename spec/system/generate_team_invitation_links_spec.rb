...
RSpec.describe 'GenerateTeamInvitationLinks', type: :system do
  ...
  context '初めて管理者が招待リンクを生成ボタンを押した時' do
    it 'URLが正常に生成される' do
      expect { click_link 'pc_screen_invitation_button' }.to change(TeamInvitation, :count).from(0).to(1)
    end
  end
  context '有効期限内(24時間以内)に管理者が招待リンクを生成ボタンを押した時' do
    ...
    it 'URLが正常に生成されず、同様のURLが表示される' do
      travel_to(2.hours.from_now) do
        expect { click_link 'pc_screen_invitation_button' }.not_to change(TeamInvitation, :count)
      end
    end
  end
  context '有効期限を過ぎてから管理者が招待リンクを生成ボタンを押した時' do
    ...
    it 'URLが正常に生成される（アップデートされる）' do
      travel_to(2.days.from_now) do
        expect { click_link 'pc_screen_invitation_button' }.to(change { TeamInvitation.last.updated_at })
      end
    end
  end
end
