...
RSpec.describe "Users::Admins::Teams::Invitations", type: :request do
  ...
  context "初めて管理者が招待リンクを生成ボタンを押した時" do
    ...
    it 'URLが正常に生成される' do
      get users_admins_teams_invitations_path(team_id:team.uuid)
      expect(response.body).to include("http://www.example.com/teams/invitations?token=#{expected_token}")
    end
  end
  ...
  context "有効期限を過ぎてから管理者が招待リンクを生成ボタンを押した時" do
    ...
    it 'URLが正常に生成される（アップデートされる）' do
      ...
      travel_to(2.days.from_now) do
        get users_admins_teams_invitations_path(team_id:team.uuid)
        expect(response.body).to include("http://www.example.com/teams/invitations?token=#{expected_updated_token}")
      end
    end
  end
end
